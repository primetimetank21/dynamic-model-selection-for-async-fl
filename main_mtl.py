...
if __name__ == "__main__":
    ...
    for _iter in range(args.epochs):
        ...
        for idx, user in enumerate(idxs_users):
            local = LocalUpdateMTL(
                args=args, dataset=dataset_train, idxs=dict_users_train[user]
            )
        ...
        for user in range(args.num_users):
            if acc_test_local[user] > best_acc[user]:
                ...
                model_save_path = os.path.join(
                    base_save_dir, "model_user{}.pt".format(user)
                )
        ...
        acc_test_avg, loss_test_avg = test_img_avg_all(
            net_glob, best_net_list, args, dataset_test
        )
        print(
    ...
    acc_test_ens_avg, loss_test, acc_test_ens_maj = test_img_ensemble_all(
        best_net_list, args, dataset_test
    )
    print(
        "Best model, acc (local): {}, acc (ens,avg): {}, acc (ens,maj): {}".format(
            best_acc, acc_test_ens_avg, acc_test_ens_maj
        )
    )

    results.append(
        np.array(
            ["Final", None, None, best_acc.mean(), acc_test_ens_avg, acc_test_ens_maj]
        )
    )
    final_results = np.array(results)
    final_results = pd.DataFrame(
