...
if __name__ == "__main__":
    ...
    for user, net_local in enumerate(net_local_list):
        ...
        ldr_train = DataLoader(
            DatasetSplit(dataset_train, dict_users_train[user]),
        ...
        for _iter in range(args.epochs):
            ...
            acc_test, loss_test = test_img_local(
                net_local, dataset_test, args, user_idx=user, idxs=dict_users_test[user]
            )
            ...
            print("User {}, Epoch {}, Acc {:.2f}".format(user, _iter, acc_test))

            if _iter > 50 and acc_test >= 99:
                break
    ...
    print(
        "Final: acc: {:.2f}, acc (avg): {:.2f}, acc (ens,avg): {:.2f}, acc (ens,maj): {:.2f}".format(
