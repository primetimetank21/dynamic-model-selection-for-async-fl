...
def load_ICU_data(path):
    ...
    input_data = pd.read_csv(
        path, names=column_names, na_values="?", sep=r"\s*,\s*", engine="python"
    ).loc[lambda df: df["race"].isin(["White", "Black"])]
    # sensitive attributes; we identify 'race' and 'sex' as sensitive attributes
    sensitive_attribs = ["race", "sex"]
    Z = input_data.loc[:, sensitive_attribs].assign(
    ...
    X = (
        input_data.drop(columns=["target", "race", "sex", "fnlwgt"])
        .fillna("Unknown")
...
def _performance_text(y_test, Z_test, y_pred, Z_pred=None, epoch=None):
    ...
    print(
        "Classifier performance: ROC AUC: %f Accuracy: %f" % (clf_roc_auc, clf_accuracy)
    )
    ...
    if Z_pred is not None:
        ...
        print(
            "Adversary performance: ROC AUC: %f acc1: %f, acc2: %f"
            % (adv_roc_auc, adv_acc1, adv_acc2)
        )
